...
Transfer.send = async (req, res, next) => {
  try {
    const { DBC, CDA, AN, ON, NRN, REF, AMT, INA } = req.body;

    const response = await request.transferMoney(
      DBC,
      CDA,
      AN,
      ON,
    ...
    logger.info(
      `Request recieved at /api/transfer | Request - ${JSON.stringify(
        req.body
      )} | Response - ${response}`
    );
...
Transfer.getAllTransactions = async (req, res, next) => {
  ...
  try {
    ...
    const result =
      await sql.query`SELECT * from stand_order_transaction where branch_id=${branchId} ORDER BY date DESC`;
    logger.info(
      `GET Request recieved at /api/transactions | userId - ${userId} | Request Object - NONE | Response - ${JSON.stringify(
        {
          status: "Success",
    ...
    console.log(error);
    logger.error(
      `Request recieved at /api/transactions | userId - ${userId} |  Request Object - NONE | Response - ${JSON.stringify(
        error
      )}`
    );
    return res.status(500).json(error);
...
Transfer.getAdminTransactions = async (req, res, next) => {
  ...
  try {
    ...
    userId = res.decoded.user_id;
    const result =
      await sql.query`SELECT * from stand_order_transaction ORDER BY date DESC`;
    logger.info(
      `GET Request recieved at /api/transactions/admin | userId - ${userId} | Request Object - NONE | Response - ${JSON.stringify(
        {
          status: "Success",
          data: result.recordset,
        }
    ...
    console.log(error);
    logger.error(
      `Request recieved at /api/transactions/admin | userId - ${userId} |  Request Object - NONE | Response - ${JSON.stringify(
        error
      )}`
    );
    return res.status(500).json(error);
...
Transfer.getInputerTransactions = async (req, res, next) => {
  ...
  try {
    ...
    userId = res.decoded.user_id;
    const result =
      await sql.query`SELECT * from stand_order_transaction where inputter_id = ${userId} ORDER BY date DESC`;
    logger.info(
      `GET Request recieved at /api/transaction/inputter | userId - ${userId} | Request Object - NONE | Response - ${JSON.stringify(
        {
          status: "Success",
          data: result.recordset,
        }
...
Transfer.GenerateReceipt = async (req, res, next) => {
  try {
    ...
    branchId = res.decoded.branch_id;
    const result =
      await sql.query`SELECT * from stand_order_transaction where transaction_id=${transId}`;
    const result2 =
      await sql.query`SELECT * from stand_order_detail where stand_order_id = ${standOrderId}`;
