...
Verify.providusInternal = async (req, res, next) => {
  try {
    ...
    logger.info(
      `Request recieved at /api/verify/internal | Request - ${JSON.stringify(
        req.body
      )} | Response - ${response}`
    );
...
Verify.externalAccount = async (req, res, next) => {
  try {
    const { DBC, AN, SAC } = req.body;

    const response = await request.externalAccountCheck(DBC, AN, SAC);
    logger.info(
      `Request recieved at /api/verify/external | Request - ${JSON.stringify(
        req.body
      )} | Response - ${response}`
    );
...
Verify.getAccountBalance = async (req, res, next) => {
  try {
    ...
    logger.info(
      `Request recieved at /api/verify/balance | Request - ${JSON.stringify(
        req.body
      )} | Response - ${response}`
    );
